...
class sentry(ShutItModule):
		...
		shutit.send('apt-get install -y openssh-server git-core libxml2-dev curl python build-essential make gcc python-dev wget')
...
def module():
        return sentry(
                'shutit.tk.sentry.sentry', 782914092.00,
