...
class android_dev(ShutItModule):
		...
		shutit.send('apt-get download fuse')
		shutit.send('dpkg-deb -x fuse_* .')
		shutit.send('dpkg-deb -e fuse_*')
		shutit.send('rm fuse_*.deb')
		shutit.send('echo -en \'#!/bin/bash\nexit 0\n\' > DEBIAN/postinst')
		shutit.send('dpkg-deb -b . /fuse.deb')
		shutit.send('dpkg -i /fuse.deb')
		shutit.send('popd')
		...
		shutit.send('pushd /')
		shutit.send('rm android-sdk_r22.3-linux.tgz && rm android-ndk-r9c-linux-x86_64.tar.bz2 && rm apache-ant-1.8.4-bin.tar.gz')
		shutit.send('popd')
