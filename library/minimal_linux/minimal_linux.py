...
class minimal_linux(ShutItModule):
	...
	def build(self, shutit):
		version = shutit.collect_config(self.module_id,'version')
		shutit.send('export KERNEL_VERSION=' + version)
		shutit.install('wget')
		shutit.install('fakeroot kernel-package xz-utils bc xorriso syslinux git vim-tiny lib32ncurses5-dev')
		shutit.send('wget -O /linux-' + version + '.tar.xz https://www.kernel.org/pub/linux/kernel/v3.x/linux-' + version + '.tar.xz') 
		shutit.send_host_file('/kernel_config.patch', 'context/kernel_config.patch')
		shutit.send('xz -d linux-' + version + '.tar.xz')
		shutit.send('tar -xvf linux-' + version + '.tar')
		shutit.send('pushd /linux-' + version)
		...
		shutit.send('cp -v /linux-' + version + '/arch/x86_64/boot/bzImage /tmp/iso/boot/vmlinuz64')
