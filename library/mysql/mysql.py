...
class mysql(ShutItModule):
	...
	def build(self, shutit):
		root_pass = shutit.collect_config(self.module_id,'root_password')
		mysql_user = shutit.collect_config(self.module_id,'mysql_user')
		mysql_user_password = shutit.collect_config(self.module_id,'mysql_user_password')
		shutit.install('sudo')
		if shutit.collect_config('target','install_type') == 'apt':
...
END""".format(root_pass), echo=False)
		...
		shutit.send("set password for " + mysql_user + "@'localhost' = password('" + mysql_user_password + "');", expect='mysql>', echo=False)
		shutit.send("set password for " + mysql_user + "@'%' = password('" + mysql_user_password + "');", expect='mysql>', echo=False)
		shutit.send(r'\q')
	...
	def remove(self, shutit):
		...
		install_type = shutit.collect_config('target','install_type')
		if install_type == 'apt':
