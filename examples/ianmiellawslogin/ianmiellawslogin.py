...
class ianmiellawslogin(ShutItModule):
    ...
    def build(self,shutit):
        ...
        shutit.send('aws ec2 run-instances --image-id ' + shutit.cfg[self.module_id]['image_id'] + ' --instance-type ' + shutit.cfg[self.module_id]['instance_type'] + ' --key-name imiell_aws_eu --security-groups launch-wizard-1')
        json_dict = json.loads(shutit.get_output())
